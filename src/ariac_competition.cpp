...
void AriacCompetition::do_kitting(std::vector<Orders> current_order) {
  ...
  if (conveyor_size == conveyor_parts.size()){
    floor_robot_->setJointValueTarget("linear_actuator_joint", -2.8);
...
void AriacCompetition::do_combined(std::vector<Orders>  current_order) {
  ...
  // Not sure what determine_agv() is doing!
...
void AriacCompetition::ceil_gripper_state_cb(const ariac_msgs::msg::VacuumGripperState::ConstSharedPtr msg){
  ...
  // RCLCPP_INFO_STREAM(get_logger(), "Ceil Robot State: " << ceil_gripper_state_.type << " " << ceil_gripper_state_.enabled << " " << ceil_gripper_state_.attached);
}
...
bool AriacCompetition::FloorRobotPickConvPart(std::vector<geometry_msgs::msg::Pose> part_pose,group3::msg::Part conv_part){
  ...
  if (part_type == ariac_msgs::msg::Part::REGULATOR){
      ...
      starting_pose.position.y += 0.01;
      starting_pose.position.z = part_pose_.position.z + part_heights_[part_type];
      waypoints.push_back(starting_pose);
      FloorRobotSetGripperState(true);
      FloorRobotMoveCartesian(waypoints, 0.2, 0.2);

  } 

  //Tune values for pump, currently wont work in all cases
  else if (part_type == ariac_msgs::msg::Part::PUMP) {
      RCLCPP_INFO_STREAM(this->get_logger(), "\033[0;91m Detetcted PUMP \033[0m");
      starting_pose.position.x = part_pose_.position.x;
      // starting_pose.position.y -= 0.16;
      starting_pose.position.z = part_pose_.position.z + part_heights_[part_type];
      tf2::Quaternion tf_q;
      tf_q.setRPY(0, 3.14159, 0);
      ...
      waypoints.push_back(BuildPose(starting_pose.position.x, starting_pose.position.y,
                                starting_pose.position.z + 0.0015, q));
      FloorRobotSetGripperState(true);
      FloorRobotMoveCartesian(waypoints, 0.4, 0.4);
      // waypoints.clear();
      // starting_pose.position.z += 0.5;
      // waypoints.push_back(starting_pose);
      // FloorRobotMoveCartesian(waypoints, 1, 0.1);
  }
  else{
  ...
  tf2::Quaternion tf_q1;
  tf_q1.setRPY(0, 3.14159, 0);
  ...
  waypoints.push_back(BuildPose(starting_pose.position.x, starting_pose.position.y,
                              starting_pose.position.z, q1));
  FloorRobotMoveCartesian(waypoints, 1, 1);
  waypoints.clear();
  
  
  // FloorRobotMoveCartesian(waypoints, 0.5, 0.5); // works for sensor
   //works for battery and sensor
  ...
  // RCLCPP_INFO_STREAM(this->get_logger(), "\033[0;91m Gripper state is  \033[0m" << std::to_string(floor_gripper_state_.enabled) << " " << std::to_string(floor_gripper_state_.attached));
  ...
  // new_pose.position.x = set_pose.position.x;
  // new_pose.position.y = set_pose.position.y;
  // new_pose.position.z = set_pose.position.z + part_heights_[part_type] + drop_height_;
  // new_pose.position.z = set_pose.position.z + 0.2;
  // waypoints.push_back(BuildPose(set_pose.position.x, set_pose.position.y,
...
void AriacCompetition::CeilRobotChangeGripper(std::string gripper_type, std::string station) {
  ...
  CeilRobotMoveCartesian(waypoints, 0.2, 0.1);
...
bool AriacCompetition::CeilRobotMoveCartesian(
    ...
    double path_fraction = ceil_robot_->computeCartesianPath(waypoints, 0.01, 0.0, trajectory);
...
bool AriacCompetition::CeilRobotPickBinPart(int part_clr,int part_type,geometry_msgs::msg::Pose part_pose,int part_quad){
  ...
  else
  {
    ...
    CeilRobotMoveCartesian(waypoints, 0.3, 0.1);
    CeilRobotSetGripperState(true);
  ...
  RCLCPP_INFO_STREAM(rclcpp::get_logger("CB"),
                        "\n\n\n\033[0;91mThe ceiling robot attached part is 1: \033[0m" << std::to_string(part_to_pick.type) << " " << std::to_string(part_to_pick.color));
  RCLCPP_INFO_STREAM(rclcpp::get_logger("CB"),
                        "\n\n\n\033[0;91mThe ceiling robot attached part is: \033[0m" << std::to_string(ceil_robot_attached_part_.type) << " " << std::to_string(ceil_robot_attached_part_.color));
...
bool AriacCompetition::CeilRobotPlacePartOnKitTray(int agv_num, int quadrant) {
  ...
  // RCLCPP_INFO_STREAM(this->get_logger(), "Detaching: " << part_name);
  // ceil_robot_->detachObject(part_name);
  ...
  // CeilRobotMoveCartesian(waypoints, 0.4, 0.1);
